  ...
  const getTodayAttendance = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8888/api/v1/accountant/getTodayAttendance/${branch_name}/${employee_ID}/${date}`
      );
  ...
  const handleLogin = async () => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:8888/api/v1/accountant/markAttendanceLogin",
        {
          branch_name,
          employee_ID,
          employee_name,
  ...
  const handleLogout = async () => {
    ...
    try {
      const response = await axios.put(
        "http://localhost:8888/api/v1/accountant/markAttendanceLogout",
        {
          branch_name,
          employee_ID,
          employee_name,
